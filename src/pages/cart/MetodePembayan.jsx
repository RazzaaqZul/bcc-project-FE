...
const MetodePembayan = () => {
  ...
  return (
    ...
    <label className="inline-flex items-center  gap-28">
      <div>
      <input type="checkbox" className="form-checkbox h-5 w-5 rounded-full " checked={isOnlinePayment} onChange={(e)=> {
        handleOnlinePayment(e)
      }}></input>
      <span className="ml-2 text-gray-700">Pembayaran Online</span>
      </div>
      <div>
      <input type="checkbox" className="form-checkbox h-5 w-5 rounded-full " checked={isOfflinePayment} onChange={(e)=>{
