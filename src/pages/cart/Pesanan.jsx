...
const Pesanan = () => {
    ...
    if (context.typePayment === true) {
      try {
        ...
        const response = await axios.post(
          `/api/v1/user/market/payment/online`,
          dataUser,
          config
        );
      ...
      try {
        ...
        const response = await axios.post(
          "/api/v1/user/market/payment/offline",
          dataUser,
          config
        );
        setDonePayment(true);
  ...
  return (
    <>
      <h1 className="text-[20px] font-bold font-mulish">Pesanan</h1>
      <section className="bg-green10 flex flex-col gap-1 h-[500px] py-3 px-2 ">
        ...
        >
          {donePayment ? <Navigate to="/riwayat"></Navigate> : ""}KONFIRMASI
          PESANAN
        </button>
