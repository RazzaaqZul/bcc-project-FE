...
const CardPesanan = ({ product }) => {
  ...
  const handleDelete = async (e) => {
    ...
    try {
      ...
      const response = await axios.delete(
        `/api/v1/user/market/cart?Product_ID=${idPost}`,
        config
      );
  ...
  return (
    <section>
        ...
        <>
          <div className="flex flex-row items-center gap-8 bg-green10 p-5 font-mulish">
            <img
              ...
              alt="item-dibeli"
              className="basis-[20%] p-2 w-[20px] h-[100px] bg-neutral20"
            />

            <div className="basis-[45%]">
              <h3 className="text-[15px] text-neutral70 font-medium">
                {category}
              </h3>
      ...
      {category && updated && (
        <>
          <h1 className="py-[46px] flex flex-row justify-center items-center">
            <Loading></Loading>
          </h1>
        </>
      )}
