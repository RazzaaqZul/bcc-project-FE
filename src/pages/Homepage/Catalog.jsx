  ...
  if ( filter ===1) {
    const handleFilterSayur = async() => {
      try {
        ...
        const response = await axios.get(`/api/v1/user/market/products/filter/${filter}`, config)
...
useEffect(()=> {
    ...
    const handleSearch = async() => {
      try {
        ...
        const response = await axios.get(`/api/v1/user/market/search?q=${search}`, config)
