    ...
    <section className="flex flex-col font-mulish mb-8 border-[1px] border-black rounded-3xl p-4  ">
      <div className="flex flex-row justify-between mb-2">
        <ul className=" basis-[80%] flex flex-row  gap-6 items-center text-[15px]">
          ...
          <li
            className={`${
              cart.status.Status === "Berlangsung" && "bg-warning"
            } ${cart.status.Status === "Berhasil" && "bg-postive"} ${
              cart.status.Status === "Tidak Berhasil" && "bg-negative"
            } rounded-3xl p-2 w-[150px] flex font-bold text-white items-center justify-center`}
          >
            {cart.status.Status}
          </li>
        ...
        <div className="basis-[20%] h-fit flex flex-row border-[1px] p-2 ml-14 border-green80 rounded-3xl justify-center gap-2">
          ...
          <button className=" text-[15px] font-extrabold tracking-widest">
            LIHAT LOKASI
          </button>
        </div>
      </div>
      <div className="flex-col flex gap-3">
        {cart.payment_product.map((item, i) => (
          <ACardTransaksi
            key={i}
            foto={item.product.product_image}
